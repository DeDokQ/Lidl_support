...
class MyApp(QWidget):
    ...
    def start_countdown(self):
        ...
        if self.current_language == "en":
            self.log_action(f"Processing in {self.countdown_time} seconds... Click again to cancel.")
        else:
            self.log_action(f"Запуск процесса через {self.countdown_time} секунды... Нажмите повторно для отмены.")
